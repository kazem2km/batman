...
import com.yara.common_test.espresso.RecyclerViewItemCountAssertion.Companion.withItemCount
...
class HomeInstrumentedTests: KoinTest {
    ...
    fun testRecyclerViewContainsItems() {
        coEvery { userRepository.getBatmansWithCache(false) } returns MutableLiveData<Resource<List<Search>>>().apply { postValue(
    ...
    fun testRefreshWhenError() {
        coEvery { userRepository.getBatmansWithCache(any()) } returns MutableLiveData<Resource<List<Search>>>().apply { postValue(
