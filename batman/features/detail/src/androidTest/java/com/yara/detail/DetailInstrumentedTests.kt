...
class DetailInstrumentedTests: KoinTest {
    ...
    private val userRepository = mockk<BatmanRepository>()
    ...
    fun testIfUserIsCorrectlyDisplayed() {
        ...
        coEvery { userRepository.getBatmanDetailWithCache(false, "fake") } returns MutableLiveData<Resource<Search>>().apply { postValue(
    ...
    fun testRefreshWhenError() {
        coEvery { userRepository.getBatmanDetailWithCache(any(), "fake") } returns MutableLiveData<Resource<Search>>().apply { postValue(
            Resource.error(Exception("no_internet"), FAKE_USERS.first())) }
        launchFragment()
    ...
    fun testNavigationToDetailImageScreen() {
        coEvery { userRepository.getBatmanDetailWithCache(false, "fake") } returns MutableLiveData<Resource<Search>>().apply { postValue(
            Resource.success(FAKE_USERS.first())) }
